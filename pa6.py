...
def make_change(total):
    ...
    def generate_changes(total, index, current_change):
        ...
        for i in range(index, len(coins)):
            if total - coins[i] >= 0:
                generate_changes(total - coins[i], i, current_change + [coins[i]])
    result = []
...
class DTree():
    def __init__(self, variable, threshold, lessequal, greater, outcome):
        '''constructing tree'''
        if (variable is None and threshold is None and lessequal is None and greater is None) \
            and (outcome is None):
            raise ValueError("Either all four of the first four arguments should be None or the last argument \
                             should be None, but not both.")
        if (variable is not None or threshold is not None or \
            lessequal is not None or greater is not None) and (outcome is not None):
            raise ValueError("Either all four of the first four arguments should be None or the last argument \
                             should be None, but not both.")
    ...
    def tuple_atleast(self):
        '''determine the minimum size of tuple depending on the variable used'''
        variables = set()
        def traverse(node):
            if node is None:
                return
    ...
    def no_repeats(self):
        '''return True if variable is inspected more than once, and False otherwise'''
